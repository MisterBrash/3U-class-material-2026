...
<ul>
    <li>Lesson: <a href="#lesson---the-optional-else-if-statement">The Optional Else-If Statement</a></li>
...
### Lesson - The Optional Else-If Statement:
...
In the above example, the computer will check the variable `selection` **FOUR** times, no matter what - even if the selection was "1". *That could be a lot of wasted checking.* **What if there were 99 options?**


<div style="text-align:center;"><p>The <code>if-statement</code> has another <em>optional</em> piece that will help with this scenario.</p><img src=".assets/else-if.png" width="500px"></div>

❗ The `else if` block(s) will *only* be checked if the condition _above it_ was `false`.
...
‼️ **Note:** You can have as many `else if` statements as you need but only *one* `if` at the beginning.
...
1. Display a menu to the user (copy this):  
    ...
    let selection = +(prompt(msg));
    ```

    Whatever number the user selects will be stored in `selection`.  

2. Based on what the user selects, use an `if-statement` and `else-if` statements to output the following (try the `alert()` function):
...
Depending on what they select, output the following:
    ...
    > 🤔 Just a thought... what if they don't enter one of _those_ choices or there's a typo?  
...
Copy this custom `randInt()` function to your `main.js`.
```JavaScript
...
Create the function `evaluate()` that does the following:

- Use `randInt()` to generate a random value from -10 to 99 and then _return_ based on the following:
    - If the value is _negative_, return -1
    - If the value is specifically zero, return 0  
    - If the value is specifically divisible by 2 _AND_ 3, return "Divisible by 6"

    However,
    - If the value is `even`, return the word "Even"
    - If the value is `odd`, return the word "Odd"  
    (Hint: "modulo" from [lesson 3.2.1](3.2.1%20-%20If.md#modulo))
...
<table style="width: 100%;">
  <tr>
    <td align="left" valign="top"><a href="3.2.1 - If.md">3.2.1 - Conditionals</a></td>
    <td align="center"><a href="3.0 - Table of Contents.md">Table of contents</a><br><br>(3.2.2 - Else-If)</td>
    <td align="right" valign="top"><a href="3.2.3 - Else.md">3.2.3 - Else</a></td>
  </tr>
</table>