...
<img src=".assets/if.png" width="500px">
</div>
...
function askUser() {
    ...
    if (age >= 19) {
        let calories = +prompt("How many calories do you burn per day?")
        if (calories < 600) {
            alert("Oh, that is not enough...")
        }
        if (calories > 3000) {
            alert("WOW! That's a heavy workout!")
        }
    }
}
...
function checkGame(gameRunning, lives) {

    if (gameRunning == true && lives == 0) {
        // Update the screen
        display("Game Over!");
...
function snowDay(snow, temperature, busesRunning) {

    if (snow >= 20 || temperature < -35 || busesRunning == false) {
        return "SNOW DAY!!!";
    }
