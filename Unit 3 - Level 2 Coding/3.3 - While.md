...
<br><br>
- Lesson: [The `While` Loop](#lesson-while)
- [Examples](#examples)
- [Practice Time!](#practice-time)
  - [Prompt Unit Cancel](#prompt-until-cancel)
  - [Countdown](#countdown)
...
// What able checking for false?
if (gamePaused == false) { }
if (!gamePaused) { }
```

Also, the `return` statement can return a complex result:
```JS
...
There is a built-in function called `isNaN()`. It returns _true_ if a value is **_not_** a number. 🤔 Ya... confusing...
```JS
let myValue = "I'm a string!"
...
// It's good for testing bad input
if (isNaN(userInput)) {
  // Do something
}
```
...
Copy & pasted code is inefficient. You should always avoid having the same code multiple times.
...
<div style="text-align:center;">
    <img src=".assets/while.jpg" width="400px" alt="While loop syntax">
</div>

<span style="color:red;">**Think of the condition _just like an if-statement_!**</span> The code will loop (repeat) and _continue_ to loop _while_ the condition is `true`. Let's look at some examples:
...
#### Prompt Until Cancel
Create the function `promptLoop()` which prompts the user and begs them to click "Cancel". If they click cancel, the function ends. If they don't, it prompts them again - begging them to click cancel.

> Want to get creative with this one? Include a counter that displays to the user the number of times you've begged them to click cancel.
...
`countdown(start, stop)`: this function will count down from the `start` value to the `stop` value.
  ...
  - When the loop finishes, the function should _return_ **the number of times it looped** (how many numbers it printed).  
    ...
    5
    4
    I looped 6 time(s)   << That's the return value
    ```
<br>
...
<table style="width: 100%;">
  <tr>
    ...
    <td align="right" valign="top"><a href="3.4 - Do While.md">3.4 - Do While</a></td>
  </tr>
</table>